...
from nfxp_model import NFXPFinLitConsBudget, Spec

# ---------- Load data ----------
def load_data(csv_path: str):
    df = pd.read_csv(csv_path)
    df = df[['id','t','a','y','x']].copy().sort_values(['id','t']).reset_index(drop=True)
    return df

def prep_grids(df: pd.DataFrame, n_a: int = 40):
    A_MIN = max(1.0, df['a'].quantile(0.02))
    A_MAX = df['a'].quantile(0.98)
    a_grid = np.linspace(A_MIN, A_MAX, n_a)
    x_grid = np.array([0.0, 0.25, 0.5, 0.75, 1.0])
    s_grid = np.array([0.6, 0.75, 0.9])   # small grid for speed
...
# ---------- Likelihood ----------
class Likelihood:
    def __init__(self, df: pd.DataFrame, model: NFXPFinLitConsBudget, x_grid: np.ndarray):
        self.df = df.copy()
        self.x_grid = x_grid
    ...
    def pack(self, theta_raw):
        # theta_raw = [beta_raw, sigma_raw, kfix_raw, tau_raw]
        beta = 0.5 + 0.49 * (1 / (1 + np.exp(-theta_raw[0])))
        sigma = 0.5 + 4.5 * (1 / (1 + np.exp(-theta_raw[1])))
        k_fix = np.exp(theta_raw[2])                    # >=0
        tau_fee = 0.0001 + 0.10 * (1 / (1 + np.exp(-theta_raw[3])))  # ~ (0.0001, 0.1001)
        return beta, sigma, k_fix, tau_fee

    def neg_loglike(self, theta_raw):
        beta, sigma, k_fix, tau_fee = self.pack(theta_raw)
        spec = Spec(beta=beta, sigma=sigma, R_f=1.02,
                    mu_lnR=np.log(1.06), sigma_lnR=0.20,
                    k_fix=k_fix, tau_fee=tau_fee)
...
# ---------- Runner ----------
def run_estimation(csv_path: str, outdir: str = "outputs_budget", n_a: int = 40,
                   maxiter: int = 250, gh_order: int = 6):
    os.makedirs(outdir, exist_ok=True)
    df = load_data(csv_path)
    a_grid, x_grid, s_grid, y_bar = prep_grids(df, n_a=n_a)
    model = NFXPFinLitConsBudget(a_grid, y_bar=y_bar, x_grid=x_grid, s_grid=s_grid, gh_order=gh_order)
    like = Likelihood(df, model, x_grid)

    theta0 = np.array([0.0, 0.0, np.log(10.0), 0.0])  # beta_raw, sigma_raw, log(k_fix), tau_raw
    res = minimize(like.neg_loglike, theta0, method='Nelder-Mead',
                   options=dict(maxiter=maxiter, xatol=1e-3, fatol=1e-3, disp=True))

    beta, sigma, k_fix, tau_fee = like.pack(res.x)
    ...
    pd.DataFrame({"param":["beta","sigma","k_fix","tau_fee"],
                  "estimate":[beta, sigma, k_fix, tau_fee]}).to_csv(os.path.join(outdir, "estimates.csv"), index=False)
    ...
    return res, (beta, sigma, k_fix, tau_fee)

if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser()
    p.add_argument("--csv", type=str, required=True)
    p.add_argument("--outdir", type=str, default="./outputs_budget")
    p.add_argument("--na", type=int, default=40)
